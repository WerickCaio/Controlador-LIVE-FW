#include "LedMatrix.h"
#include "../Hardware/Hal.h" // Acesso à camada de hardware
#include "Drawing.h"         // Acesso às fontes e números

// A nossa memória de vídeo (VRAM) intocável
uint64_t ledPanelRgb[2][48];

void Display_PutPixel(int x, int y, int color) { 
    if (x < 0 || x > 127 || y < 0 || y > 15) return; // Proteção contra crash
    ...
    switch (color) {
        case red:
            multiplier = 0; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] |= ((unidade << tempar)); break;
        case green:
            multiplier = 1; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] |= ((unidade << tempar)); break;
        case blue:
            multiplier = 2; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] |= ((unidade << tempar)); break;
        case yellow:
            multiplier = 0; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] |= ((unidade << tempar));
            multiplier = 1; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] |= ((unidade << tempar)); break;
        case cyan:
            multiplier = 1; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] |= ((unidade << tempar));
            multiplier = 2; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] |= ((unidade << tempar)); break;
        case purple:
            multiplier = 0; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] |= ((unidade << tempar));
            multiplier = 2; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] |= ((unidade << tempar)); break;
...
void Display_ClearPixel(int x, int y, int color) {
    ...
    switch (color) {
        ...
        case white:
            ...
            multiplier = 1; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] &= (~(unidade << tempar));
            multiplier = 2; ledPanelRgb[1 - x / 64][y + (multiplier * 16)] &= (~(unidade << tempar)); break;
        // Adicione outras cores do clear se necessário
    }
}
...
void Display_Clear() {
    for (size_t j = 0; j < 128; j++) {
        for (size_t i = 0; i < 16; i++) {
            Display_ClearPixel(j, i, white); // Apaga tudo
        }
    }
}
...
void Display_Init() {
    Display_Clear();
    // Desenha as bordas brancas (antigo initializerPanel)
    for (int i = 0; i < 16; i++) {
    ...
    for (int i = 0; i < 64; i++) {
        ...
        Display_PutPixel(64 + i, 15, white);
    }
}
...
void Display_ClearCharArea(int x, int y, int color) {
    for (size_t i = 0; i < 10; i++) {
        for (size_t j = 0; j < 5; j++) {
            // Simula a antiga vassoura apagando o bloco 10x5
...
// ======================================================================
// A VARREDURA - O coração gráfico usando a HAL
// ======================================================================
void Display_Update() {
    HAL_DisableLines(); // digitalWrites(HIGH) da vida agora ficam na HAL
